...
중요: 현재 저장소는 “제너레이터가 소스 오브 트루스”입니다. 공유 자산(CSS/폰트/partials)은 `generator/src/main/resources/templates/`에 보관되며, 릴리스 스켈레톤과 빌드 산출(dist)은 제너레이터를 통해 생성합니다. 루트의 HTML은 샘플/데모 용도로만 유지되며, 실제 배포는 제너레이터의 템플릿을 기준으로 합니다.

## 1) 프로젝트 목표와 범위
- 범위: 정적 HTML + CSS. 빌드 도구/JS 프레임워크 사용 금지.
...
## 14) Generator 서브프로젝트 안내
- 경로: `generator/`
- 목적: 본 저장소의 정적 템플릿을 CLI로 초기화/빌드하는 Java(GraalVM Native) 생성기 구현.
- 적용 규칙: `generator/AGENTS.md`가 해당 디렉토리 트리에 한해 우선 적용됩니다.
- 예외: 이 서브프로젝트는 빌드 도구/네이티브 이미지를 사용합니다. 루트 규칙의 “빌드 도구 금지”는 `generator/`에는 적용되지 않습니다.
