...
public class Main {
    ...
    static void doInit(String[] args) throws Exception {
        ...
        if (dest == null) {
            System.err.println("usage: init <dir> [--dry-run] [--verbose]");
            System.exit(2);
        }
    ...
    static void doImportMd(String[] args) throws Exception {
        ...
        if (md == null) {
            System.err.println("usage: import:md --src <md_dir> [--root dir] [--dry-run] [--verbose]");
            System.exit(2);
        }
    ...
    static void doSample(String[] args) throws Exception {
        Path out = Path.of("sample-site");
        boolean build = false, dry = false, verb = false;
        for (int i = 1; i < args.length; i++) {
            switch (args[i]) {
                case "--out" -> out = Path.of(args[++i]);
